...
def trust_score_node(state: InfluencerState) -> InfluencerState:
    ...
    # Signal 1: Engagement rate (30pts)
    sig1 = engagement_score(er, tier) * 30

    # Signal 2: Followers/following ratio (20pts)
    ratio = followers / following
    if ratio >= 3.0:
        sig2 = 20.0
    elif ratio >= 1.0:
        sig2 = 10.0 + 10.0 * (ratio - 1.0) / 2.0
    elif ratio >= 0.3:
        sig2 = 10.0 * (ratio / 1.0)
    else:
        sig2 = 2.0

    # Signal 3: Fake follower estimate from igaudit model (25pts)
    fake_prob = state.get("fake_probability", 0.2)
    sig3 = (1.0 - fake_prob) * 25

    # Signal 4: Post consistency (10pts)
    ...
    # Signal 5: Profile completeness (10pts)
    completeness = 0
    if has_pic:
        completeness += 4
    if bio_len > 20:
    ...
    if has_url:
        completeness += 3
    sig5 = float(completeness)

    # Signal 6: Account age (5pts)
    if age_months >= 24:
        sig6 = 5.0
    elif age_months >= 12:
        sig6 = 3.5
    ...
    raw = sig1 + sig2 + sig3 + sig4 + sig5 + sig6
    ...
    breakdown = {
        ...
        "consistency": round(sig4, 1),
        "completeness": round(sig5, 1),
        "age":         round(sig6, 1),
    }
